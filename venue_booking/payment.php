  ...
  <div class="modal">
    <div class="modal__container">
      ...
      <div class="modal__content">
        <h2><font color="white">Your payment details</font></h2>

        <form method="POST" form action="payment.php">
		<br>
          <ul class="form-list">
           ...
           <li class="form-list__row">
			...
			<br>
			<label><?php echo"<h2>Total Payment : ".$hall_rent." Rs/-</h2>" ?> 
            </li></label><br>
			<li class="form-list__row">
            ...
            <li>
              <input type="submit" class="button" name="submit" value="Pay Now">
            </li>
          </ul>
        </form>
...
echo'<font color="white">';
 ...
 $reg_user_query = $conn->query("select * from booking order by booking_id desc");
     
            $reg_user_res = $reg_user_query->fetch_array(MYSQLI_BOTH);
            $temp = $reg_user_res['booking_id'];
            $i=2;
            $str ="";
            while($temp[$i] != 'B')
            {
...
$query1=$conn->query("insert into booking(booking_id,hall_id,user_id,status,date)values('$booking_id','$hall_id', '$user_id','0','$date')"); 


$query2=$conn->query("insert into payment(user_id,transaction_id,paid_money,date,
bank_name,booking_id,hall_id)values('$user_id','$payment_id','$hall_rent','$date','$bank_name','$booking_id','$hall_id') ");

//payment(user_id,transaction_id,paid_money,date,bank_name,booking_id,hall_id)values('$user_id','$payment_id','20000','$date','$bank_name',$booking_id','$hall_id') ");


if($query2 && $query1)
	echo "<center><font color='white'>yes hall booked";
else
	echo "not booked";
